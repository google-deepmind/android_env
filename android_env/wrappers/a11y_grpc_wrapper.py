...
class A11yGrpcWrapper(base_wrapper.BaseWrapper):
  ...
  * If someone *has* called `task_extras()` since last step, the newly fetched
  ...
  def __init__(
    ...
    # TODO(b/293157187): Ocasionally, the A11yForwarder will fail to provide
    # a11y information, which is communicated through task_extras in the form of
    # an exception. When this is caused by the agent disabling the network, the
    # wrapper will attempt to re-enable networking. If re-enabling the network
    # does not solve the issue, there might be other issues in the gRPC
    # connection that needs to be resolved.
  ...
  def _install_a11y_forwarding_apk(self) -> None:
    ...
    a11y_fwd_apk = _get_a11y_fwd_apk()
