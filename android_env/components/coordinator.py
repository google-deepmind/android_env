...
class Coordinator():
  ...
  def action_spec(self) -> Dict[str, dm_env.specs.Array]:
    return specs.base_action_spec(num_fingers=self._num_fingers)
  ...
  def _reset_state(self):
    ...
    try:
      ...
      self._stats['restart_count_simulator_reset'] += 1
      self._simulator_healthy = False
  ...
  def rl_step(self,
              agent_action: Optional[Dict[str, np.ndarray]]) -> dm_env.TimeStep:
    ...
    if agent_action is None:  # An RL reset.
      self._reset_state()
    # If the action is a TOUCH or LIFT, send it to the simulator.
    elif agent_action['action_type'] != action_type_lib.ActionType.REPEAT:
    ...
    try:
      ...
      self._simulator_healthy = False
      simulator_signals = {'simulator_healthy': np.array(False, dtype=bool)}

    return self._task_manager.rl_step(agent_action, simulator_signals)

  def _gather_simulator_signals(self) -> Dict[str, np.ndarray]:
    """Gathers data from various sources to assemble the RL observation.

    Returns:
      A collection of data points from the simulator. One crucial entry is
        `"simulator_healthy": bool` which indicates whether the simulator
        is healthy. If that's False, all other data may be unavailable or
        invalid.
    """

    if not self._simulator_healthy:
      return {'simulator_healthy': np.array(False, dtype=bool)}
    ...
    return {
        # Whether the simulator is healthy.
        'simulator_healthy': np.array(self._simulator_healthy, dtype=bool),
  ...
  def _wait_for_device(self, max_tries: int) -> None:
    ...
    for _ in range(max_tries):
      ready = _check_device_is_ready(required_services)
      if ready:
