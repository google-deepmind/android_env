...
class AdbControllerTest(parameterized.TestCase):
  ...
  def test_start_screen_pinning_task_not_found(self,
    ...
    self.assertRaises(
        errors.AdbControllerScreenPinningError,
        self._adb_controller.start_screen_pinning,
        'my.app.CoolActivity', _TIMEOUT)
...
"""
  ...
  def test_uninstall_package_installed(self):
    self._mock_execute_command.return_value = b"""
package:foo
package:bar
package:baz
"""
    self._adb_controller.uninstall_package('foo', timeout=_TIMEOUT)
    self._mock_execute_command.assert_has_calls([
        mock.call(
            self._adb_controller, ['shell', 'pm', 'list', 'packages'],
            timeout=_TIMEOUT),
        mock.call(self._adb_controller, ['uninstall', 'foo'], _TIMEOUT),
    ])

  def test_uninstall_package_not_installed(self):
    self._mock_execute_command.return_value = b"""
package:bar
package:baz
"""
    self._adb_controller.uninstall_package('foo', timeout=_TIMEOUT)
    # Uninstall command not called.
    self._mock_execute_command.assert_called_once_with(
        self._adb_controller, ['shell', 'pm', 'list', 'packages'],
        timeout=_TIMEOUT)
